...
const ShareButton: React.FC<ShareButtonProps> = ({title}) => {
  ...
  const shareToSocial = (platform: SocialPlatform) => {
    const shareText = `I've just ranked "${title}" on AmazingTier. What do you think? 🤔`;
  ...
  const copyAsMarkdown = () => {
    const shareText = `I've just ranked **${title}** on AmazingTier.
What do you think? 🤔
[Check it out on AmazingTier!](${window.location.href})
-# The link is shortened for brevity. Always double-check before clicking! It should start with \`https://www.AmazingTier.com/\`.
`;
